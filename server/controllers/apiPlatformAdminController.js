...
exports.createKey = asyncHandler(async (req, res) => {
  const { apiKey, rawKey } = await apiKeyService.createApiKey({ ...req.body, actor: req.user });
...
exports.revokeKey = asyncHandler(async (req, res) => {
  const apiKey = await apiKeyService.revokeApiKey(req.params.id, { actor: req.user, reason: req.body.reason });
  res.json({ status: 'success', data: { apiKey } });
});

exports.rotateKey = asyncHandler(async (req, res) => {
  const { apiKey, rawKey } = await apiKeyService.rotateApiKey(req.params.id, { actor: req.user, reason: req.body.reason });
  res.json({ status: 'success', data: { apiKey, rawKey } });
});

exports.getCallLogs = asyncHandler(async (req, res) => {
  const filter = {};
  if (req.query.apiKeyId) filter.apiKey = req.query.apiKeyId;
...
exports.getWebhookSubscriptions = asyncHandler(async (req, res) => {
  const subscriptions = await WebhookSubscription.find().select('-secret').populate('apiKey', 'name keyPrefix').sort({ createdAt: -1 }).lean();
