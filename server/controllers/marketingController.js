...
exports.previewSegment = asyncHandler(async (req, res) => {
  const ids = await segmentService.resolveSegment(req.params.key, req.query);
  res.json({ status: 'success', data: { segmentKey: req.params.key, count: ids.length, sampleIds: ids.slice(0, 20) } });
});

// ── Modèles (Phase 6) ───────────────────────────────────────────────────
exports.listTemplates = asyncHandler(async (_req, res) => {
  res.json({ status: 'success', data: { templates: await templateService.listActiveTemplates() } });
});

exports.templateHistory = asyncHandler(async (req, res) => {
  res.json({ status: 'success', data: { history: await templateService.listTemplateHistory(req.params.family) } });
});

exports.createTemplateVersion = asyncHandler(async (req, res) => {
  const template = await templateService.createTemplateVersion({ ...req.body, actor: req.user });
  res.status(201).json({ status: 'success', data: { template } });
});

exports.activateTemplate = asyncHandler(async (req, res) => {
  const template = await templateService.activateTemplate(req.params.id, { actor: req.user });
...
// ── Campagnes (Phase 4) ─────────────────────────────────────────────────
exports.listCampaigns = asyncHandler(async (_req, res) => {
  res.json({ status: 'success', data: { campaigns: await campaignService.listCampaigns() } });
});

exports.createCampaign = asyncHandler(async (req, res) => {
  const campaign = await campaignService.createCampaign({ ...req.body, actor: req.user, req });
  res.status(201).json({ status: 'success', data: { campaign } });
});

exports.approveCampaign = asyncHandler(async (req, res) => {
  const campaign = await campaignService.approveCampaign(req.params.id, { actor: req.user, req });
  res.json({ status: 'success', data: { campaign } });
});

exports.cancelCampaign = asyncHandler(async (req, res) => {
  const campaign = await campaignService.cancelCampaign(req.params.id, { actor: req.user, reason: req.body.reason, req });
  res.json({ status: 'success', data: { campaign } });
});

exports.sendCampaign = asyncHandler(async (req, res) => {
  const campaign = await campaignService.sendCampaign(req.params.id, { actor: req.user, req });
  res.json({ status: 'success', data: { campaign } });
});
