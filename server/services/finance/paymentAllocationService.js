...
async function allocatePaymentToDocumentCore({ paymentId, documentId, amountMinor, businessOperationKey, actor, session, transactional, faultInjector }) {
  ...
  let allocation;
  const allocationData = { tenant: document.tenant || payment.tenant || actor.platformTenant?._id || actor.platformTenant || null, financialPayment: paymentId, financialDocument: documentId, domain: document.domain, establishmentType: document.establishmentType, establishmentId: document.establishmentId, currency: document.currency, amountMinor, businessOperationKey, allocatedBy: actor.id || actor._id, metadata: { payloadHash } };
