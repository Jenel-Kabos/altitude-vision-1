...
async function resolveCustomerId(event) {
  if (!event.platformTenantId) return null;
  if (event.entityType === 'crmCustomer' && event.entityId) {
    const customer = await CrmCustomer.findOne({ _id: event.entityId, tenant: event.platformTenantId }).select('_id').lean();
    return customer ? String(customer._id) : null;
  }
  if (event.recipient) {
    const customer = await CrmCustomer.findOne({ tenant: event.platformTenantId, identityKeys: `user:${event.recipient}` }).select('_id').lean();
...
async function createCrmActivity(event, params = {}) {
  ...
  }, actor, { tenantId: event.platformTenantId });
...
async function closeOpportunityWon(event, params = {}) {
  ...
  const actor = resolveActor(event);
  if (!actor) return { status: 'skipped', reason: 'no_actor' };
  const openOpportunity = await CrmOpportunity.findOne({ tenant: event.platformTenantId, customer: customerId, outcome: 'open' }).sort({ updatedAt: -1 });
  ...
  await setOpportunityOutcome(openOpportunity._id, { outcome: 'won', reason: params.reason || 'Contrat signé' }, actor, { tenantId: event.platformTenantId });
...
async function createNotification(event, params = {}) {
  ...
  await notify({
    ...
    platformTenantId: event.platformTenantId,
  });
...
async function sendMarketingMessage(event, params = {}) {
  ...
  const template = await MarketingTemplate.findOne({ tenant: event.platformTenantId, family: params.templateFamily, status: 'active' });
  if (!template) return { status: 'skipped', reason: 'no_active_template' };
  const customer = await CrmCustomer.findOne({ _id: customerId, tenant: event.platformTenantId }).select('emails identityKeys displayName firstName').lean();
  ...
  await MarketingSend.create({
    tenant: event.platformTenantId, workflowRuleId: params.ruleId || event.type, template: template._id, channel: params.channel || template.channel,
