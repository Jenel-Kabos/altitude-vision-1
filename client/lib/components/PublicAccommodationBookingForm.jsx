...
export default function PublicAccommodationBookingForm({ accommodation, user }) {
  ...
  if (success) return <div role="status" className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><strong>Votre demande est maintenue pendant 2 heures.</strong><p>Elle sera confirmée seulement après validation du paiement d’une nuitée.</p></div>;
  return <form onSubmit={submit} className="space-y-3" aria-label="Demander une réservation">
    ...
    {quote?.pricing && <div className="rounded-lg bg-white/10 p-3 text-sm"><p>{quote.pricing.nights} nuit(s) × {money.format(quote.pricing.nightlyRate)}</p>{quote.pricing.cleaningFee > 0 && <p>Frais : {money.format(quote.pricing.cleaningFee)}</p>}<strong>Total : {money.format(quote.pricing.total)}</strong><p>À payer pour confirmer : {money.format(quote.pricing.requiredToConfirm)}</p><p>Reste après garantie : {money.format(Math.max(0, quote.pricing.total - quote.pricing.requiredToConfirm))}</p><p className="mt-2 text-xs">La première nuitée constitue une garantie non remboursable si vous annulez, quelle que soit la date d’annulation.</p>{!quote.available && <p role="alert" className="text-red-300">Période indisponible</p>}</div>}
    ...
    <p className="text-xs opacity-70">La demande bloque les dates pendant 2 heures et reste en attente du paiement de la garantie.</p>
  </form>;
}
