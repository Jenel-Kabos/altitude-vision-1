...
const META = {
  ...
  arrondissement: ['Arrondissement', 1], phone: ['Téléphone', 1], roomCategories: ['Catégories de chambres', 3],
  hotelServices: ['Services', 5], checkInTime: ["Heure d’arrivée", 6], checkOutTime: ['Heure de départ', 6], images: ['Photos', 7],
};

export function getHotelWizardFieldMeta(field) {
  if (META[field]) return { label: META[field][0], step: META[field][1] };
  if (field.startsWith('roomCategories.')) return { label: field.includes('ratePlans') ? 'Tarif de catégorie' : 'Catégorie de chambre', step: field.includes('ratePlans') ? 4 : 3 };
...
export function validateHotelWizard(form, onlyStep = null) {
  ...
  if (include(3)) Object.assign(errors, validateHotelRoomCategories(form.roomCategories));
  if (include(4)) Object.assign(errors, validateHotelRates(form.roomCategories));
  if (include(5) && !Object.values(form.hotelServices || {}).some(Boolean)) errors.hotelServices = 'Sélectionnez au moins un service.';
  if (include(6)) {
  ...
  if (include(7) && !(form.images?.length > 0)) errors.images = 'Ajoutez au moins une photo.';
  return errors;
}
