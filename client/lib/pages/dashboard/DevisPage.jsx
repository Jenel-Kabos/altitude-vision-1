...
import {
  FileText, Mail, Home, MapPin, Maximize2, Banknote, Layers,
  Loader2, AlertTriangle, CheckCircle2, XCircle, PlayCircle, StickyNote,
} from "lucide-react";
import { getAllDevis, updateDevis } from "../../services/devisService";

const STATUTS = ["Tous", "En attente", "En cours", "Traité", "Annulé"];
...
const STATUT_STYLE = {
  ...
  "Annulé":     { bg: "bg-red-100",    text: "text-red-700",    dot: "bg-red-500"    },
};
...
const DevisPage = () => {
  ...
  const handleUpdate = async (id, data) => {
    setSubmitting(id);
    try {
      const updated = await updateDevis(id, data);
      setDevis(prev => prev.map(d => d._id === id ? { ...d, ...updated } : d));
  ...
  const countByStatut = (s) => devis.filter(d => d.statut === s).length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-4 sm:p-8 font-sans">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl sm:text-4xl font-black text-gray-800">Demandes de devis — Gestion locative</h1>
            <p className="text-gray-500 text-sm mt-0.5">
              {devis.length} demande{devis.length !== 1 ? "s" : ""} au total
            </p>
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-4">
            {filtered.map(d => {
              const isBusy = submitting === d._id;
              const noteValue = notes[d._id] ?? d.noteInterne ?? "";
                ...
                <div key={d._id}
                  ...
                  <div className="p-5">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                      <p className="font-bold text-gray-800 text-base leading-tight">{d.nom}</p>
                      <StatutBadge statut={d.statut} />
                    </div>

                    <a href={`mailto:${d.email}`}
                      className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:underline mb-1">
                      <Mail className="w-3.5 h-3.5" />
                      {d.email}
                    </a>
                    {d.telephone && (
                      <p className="text-sm text-gray-600 mb-2">📞 {d.telephone}</p>
                    )}
                    ...
                    <div className="flex flex-wrap gap-3 mt-1 text-xs text-gray-600">
                      ...
                      {d.loyerSouhaite && (
                        <span className="flex items-center gap-1 bg-gray-50 px-2.5 py-1 rounded-full">
                          ...
                          {d.loyerSouhaite.toLocaleString('fr-FR')} FCFA
                        </span>
                      )}
                      <span className="flex items-center gap-1 bg-gray-50 px-2.5 py-1 rounded-full">
                        <Layers className="w-3 h-3" />
                        {d.nbBiens || 1} bien{(d.nbBiens || 1) > 1 ? 's' : ''} à gérer
                      </span>
                    </div>

                    {d.message && (
                      <p className="text-sm text-gray-700 whitespace-pre-wrap bg-gray-50 rounded-lg p-3 mt-3">
                        {d.message}
                      </p>
                    )}

                    <p className="text-gray-400 text-xs mt-2">
                      Reçue le {formatDate(d.createdAt)}
                      {d.traitePar?.name && <span> · Traitée par {d.traitePar.name}</span>}
                    </p>
                  </div>
                  ...
                  <div className="border-t border-gray-100 px-5 py-4 space-y-3">
                    <div className="flex gap-2 flex-wrap">
                      {d.statut === 'En attente' && (
                        <button
                          onClick={() => handleUpdate(d._id, { statut: 'En cours' })}
                          disabled={isBusy}
                      ...
                      {d.statut !== 'Traité' && d.statut !== 'Annulé' && (
                        <button
                          onClick={() => handleUpdate(d._id, { statut: 'Traité' })}
                          disabled={isBusy}
                      ...
                      {d.statut !== 'Annulé' && (
                        <button
                          onClick={() => handleUpdate(d._id, { statut: 'Annulé' })}
                          disabled={isBusy}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <button
                        ...
                        disabled={isBusy || notes[d._id] === undefined}
