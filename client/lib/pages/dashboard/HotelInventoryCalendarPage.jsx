...
const statusLabels = { pending: "En attente", confirmed: "Confirmée", checked_in: "En séjour", checked_out: "Terminée", cancelled: "Annulée" };
const assignmentLabels = { unassigned: "Non affectée", partially_assigned: "Partielle", fully_assigned: "Complète" };
const sameDay = (value, expected) => value && iso(new Date(value)) === expected;

export default function HotelInventoryCalendarPage() {
  ...
  const categories = useMemo(() => [...new Map(data.days.map((day) => [String(day.roomCategory), { id: String(day.roomCategory), name: day.categoryName }])).values()], [data.days]);
  ...
  const visibleReservations = useMemo(() => data.reservations.filter((reservation) => {
    const rooms = reservation.assignedRooms || []; const guest = reservation.guest || {};
    const haystack = [reservation.reference, guest.firstName, guest.lastName, guest.phone, guest.email, reservation.roomCategory?.name, ...rooms.map((room) => room?.roomNumber)].filter(Boolean).join(" ").toLowerCase();
    return (!filters.category || String(reservation.roomCategory?._id || reservation.roomCategory) === filters.category)
      && (!filters.floor || rooms.some((room) => String(room?.floor) === filters.floor))
      && (!filters.status || reservation.status === filters.status)
      && (!filters.assignment || reservation.assignmentState === filters.assignment)
      && (!filters.arrivals || sameDay(reservation.checkInDate, today)) && (!filters.departures || sameDay(reservation.checkOutDate, today))
      && (!filters.unassigned || reservation.assignmentState !== "fully_assigned") && (!query || haystack.includes(query));
  }), [data.reservations, filters, query]);
  const visibleCategories = categories.filter((category) => !filters.category || category.id === filters.category);
  const resetFilters = () => setFilters({ category: "", floor: "", status: "", assignment: "", arrivals: false, departures: false, unassigned: false, outOfService: false, stopSell: false, blocked: false, query: "" });

  return <DashboardPage className="text-gray-900 dark:text-gray-100">
    <DashboardPageHeader icon={CalendarDays} title="Calendrier d’inventaire" description="Pilotez disponibilités, affectations et opérations sur une même période." />
    <DashboardToolbar label="Navigation de période"><button onClick={() => move(-1)} className="border rounded px-3 py-2 focus:ring-2" aria-label="Période précédente">←</button><button onClick={() => setFrom(today)} className="border rounded px-3 py-2">Aujourd’hui</button><button onClick={() => move(1)} className="border rounded px-3 py-2 focus:ring-2" aria-label="Période suivante">→</button><button aria-pressed={view === "week"} onClick={() => setView("week")} className="border rounded px-3 py-2">Semaine</button><button aria-pressed={view === "month"} onClick={() => setView("month")} className="border rounded px-3 py-2">Mois</button><input aria-label="Date de début" type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="border rounded px-3 py-2 bg-transparent" /></DashboardToolbar>
    <DashboardToolbar className="grid sm:grid-cols-2 lg:grid-cols-4" label="Filtres du calendrier">
      <input aria-label="Rechercher une réservation" placeholder="Référence, client, téléphone, email, chambre…" value={filters.query} onChange={(e) => setFilters((f) => ({ ...f, query: e.target.value }))} className="border rounded px-3 py-2 bg-transparent lg:col-span-2" />
      <select aria-label="Filtrer par catégorie" value={filters.category} onChange={(e) => setFilters((f) => ({ ...f, category: e.target.value }))} className="border rounded px-3 py-2 bg-white dark:bg-gray-900"><option value="">Toutes les catégories</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
      <select aria-label="Filtrer par étage" value={filters.floor} onChange={(e) => setFilters((f) => ({ ...f, floor: e.target.value }))} className="border rounded px-3 py-2 bg-white dark:bg-gray-900"><option value="">Tous les étages</option>{floors.map((floor) => <option key={floor} value={floor}>Étage {floor}</option>)}</select>
      <select aria-label="Filtrer par statut" value={filters.status} onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value }))} className="border rounded px-3 py-2 bg-white dark:bg-gray-900"><option value="">Tous les statuts</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
      <select aria-label="Filtrer par affectation" value={filters.assignment} onChange={(e) => setFilters((f) => ({ ...f, assignment: e.target.value }))} className="border rounded px-3 py-2 bg-white dark:bg-gray-900"><option value="">Toutes les affectations</option>{Object.entries(assignmentLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
      {[['arrivals', 'Arrivées aujourd’hui'], ['departures', 'Départs aujourd’hui'], ['unassigned', 'Affectations incomplètes'], ['outOfService', 'Hors service'], ['stopSell', 'Stop-sell'], ['blocked', 'Stock bloqué']].map(([key, label]) => <label key={key} className="flex items-center gap-2 min-h-11"><input type="checkbox" checked={filters[key]} onChange={(e) => setFilters((f) => ({ ...f, [key]: e.target.checked }))} />{label}</label>)}
      <button onClick={resetFilters} className="border rounded px-3 py-2">Réinitialiser</button>
    </DashboardToolbar>
    <aside aria-label="Légende" className="flex flex-wrap gap-3 text-xs mb-4"><span>⏳ En attente</span><span>✅ Confirmée</span><span>🔑 En séjour</span><span>🧹 Nettoyage</span><span>🔎 Inspection</span><span>⛔ Hors service</span><span>⚠ Affectation partielle</span><span>🚫 Vente fermée</span></aside>
