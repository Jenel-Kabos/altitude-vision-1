...
describe('tenantApplicationReviewService', () => {
  ...
  test('liste avec filtres/pagination côté serveur et normalise id', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/platform-tenants/applications', { params: { page: 2, limit: 20, status: 'SUBMITTED' } });
  });
  ...
  test('approve POST est vide de toute autorité client', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/platform-tenants/applications/app-1/approve');
  });
  ...
  test('reject et request-changes utilisent uniquement leurs décisions certifiées', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/platform-tenants/applications/app-1/reject', { reason: 'Motif public.' });
    ...
    expect(api.post).toHaveBeenCalledWith('/platform-tenants/applications/app-1/request-changes', payload);
  });
  ...
  test('document privé passe par review-documents et URL éphémère révoquée', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/platform-tenants/applications/app-1/review-documents/doc-1', { responseType: 'blob' });
