  ...
  return new ImageResponse(
    (
      <div style={{ display:'flex', width:'1200px', height:'630px', background:'#0A0C0F' }}>
        ...
        {/* Background gradient — blue tint left */}
        <div style={{ display:'flex', position:'absolute', inset:'0', background:'linear-gradient(130deg,#0B1929 0%,#0A0C0F 60%)' }} />

        {/* Top blue accent bar */}
        <div style={{ display:'flex', position:'absolute', top:0, left:0, right:0, height:'5px', background:BLUE }} />

        {/* Left vertical accent */}
        <div style={{ display:'flex', position:'absolute', top:'56px', left:0, width:'4px', height:'518px', background:`linear-gradient(to bottom,${BLUE},transparent)` }} />

        {/* Decorative ring top-right */}
        <div style={{ display:'flex', position:'absolute', top:'-80px', right:'-80px', width:'300px', height:'300px', borderRadius:'50%', border:`1px solid rgba(46,123,181,0.12)` }} />
        <div style={{ display:'flex', position:'absolute', top:'-20px', right:'-20px', width:'180px', height:'180px', borderRadius:'50%', border:`1px solid rgba(46,123,181,0.07)` }} />
        ...
        <div style={{ display:'flex', flexDirection:'column', flex:1, padding:'56px 80px 48px 96px', zIndex:1 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'44px' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'6px 16px', borderRadius:'40px', border:`1px solid rgba(46,123,181,0.25)`, background:'rgba(46,123,181,0.08)' }}>
              <span style={{ color:BLUE, fontSize:'11px', letterSpacing:'0.20em', textTransform:'uppercase', fontWeight:500 }}>Altimmo — Immobilier</span>
            </div>
          </div>

          {/* Eyebrow */}
          <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'16px' }}>
            <div style={{ display:'flex', width:'18px', height:'1px', background:BLUE }} />
            <span style={{ color:BLUE, fontSize:'11px', letterSpacing:'0.22em', textTransform:'uppercase', fontWeight:500 }}>Résidence & Investissement</span>
          </div>
          ...
          <div style={{ display:'flex', color:'rgba(232,228,220,0.40)', fontSize:'20px', fontWeight:300, lineHeight:1.5, marginBottom:'auto' }}>
            Biens vérifiés, prix transparents — des experts qui connaissent chaque quartier.
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:'18px', borderTop:'1px solid rgba(232,228,220,0.06)' }}>
            <div style={{ display:'flex', gap:'24px' }}>
              {[['150+','Biens sélectionnés'], ['5+','Années au Congo']].map(([n,l]) => (
                <div key={n} style={{ display:'flex', flexDirection:'column' }}>
                  <span style={{ color:BLUE, fontSize:'26px', fontWeight:700, lineHeight:1 }}>{n}</span>
                  <span style={{ color:'rgba(232,228,220,0.28)', fontSize:'11px', letterSpacing:'0.10em', textTransform:'uppercase' }}>{l}</span>
                </div>
              ))}
            </div>
            <span style={{ color:`rgba(46,123,181,0.45)`, fontSize:'13px' }}>altitudevision.agency</span>
          </div>
        </div>
      </div>
    ),
