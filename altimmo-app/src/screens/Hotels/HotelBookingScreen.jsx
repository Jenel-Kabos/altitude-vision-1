...
import { formatMealPlan, formatCancellationTerms } from '../../utils/hotelCommercialConditions';
...
export default function HotelBookingScreen({ navigation, route }) {
  ...
  // PHASE-H1 — deep-link optionnel depuis HotelDetailScreen : si un
  // `hotelId` est fourni, la recherche/sélection (étape 1) est sautée.
  // Aucun appel existant ne passe ce paramètre aujourd'hui (HotelBooking
  // est toujours ouvert sans params depuis MyHotelReservationsScreen) :
  // comportement strictement additif, jamais un changement du flux par défaut.
  const deepLinkedHotelId = route?.params?.hotelId;
  // PHASE-H2 — contexte complet optionnel depuis HotelDetailScreen (une
  // offre déjà choisie sur une recherche réelle) : catégorie/tarif ne
  // doivent JAMAIS être silencieusement changés dans ce cas (mission §15).
  const lockedContext = route?.params?.roomCategoryId && route?.params?.ratePlanId ? route.params : null;
  ...
  const chooseHotel = async (hotel) => {
    setLoading(true);
    try {
      const data = await getPublicHotel(hotel._id);
      setHotelData(data);
      if (lockedContext) {
        // PHASE-H2 — catégorie/tarif verrouillés depuis une offre réelle déjà
        // recherchée : jamais re-choisis silencieusement, jamais un fallback
        // sur une autre catégorie si l'identifiant ne correspond plus.
        const category = (data.categories || []).find((cat) => String(cat._id) === String(lockedContext.roomCategoryId));
        const rate = category?.rates?.find((r) => String(r._id) === String(lockedContext.ratePlanId));
        if (category && rate) {
          setSelectedCategory(category); setSelectedRate(rate); setLocked(true);
          setForm((current) => ({
            ...current,
            checkInDate: lockedContext.checkInDate || current.checkInDate,
            checkOutDate: lockedContext.checkOutDate || current.checkOutDate,
            roomsCount: String(lockedContext.roomsCount || current.roomsCount),
            adults: String(lockedContext.adults || current.adults),
            children: String(lockedContext.children ?? current.children),
          }));
          setStep(5);
        } else {
          Alert.alert('Offre indisponible', 'Cette offre n’est plus disponible. Sélectionnez à nouveau vos dates.');
          setStep(2);
        }
      } else {
        setStep(2);
      }
    } catch (error) { Alert.alert('Hôtel indisponible', error.response?.data?.message || 'Impossible de charger cet hôtel.'); } finally { setLoading(false); }
  };
  useEffect(() => { if (deepLinkedHotelId && !hotelData) chooseHotel({ _id: deepLinkedHotelId }); }, [deepLinkedHotelId]); // eslint-disable-line react-hooks/exhaustive-deps
  const verify = async () => {
  ...
  const reset = () => { requestId.current = null; setSuccess(null); setHotelData(null); setSelectedCategory(null); setSelectedRate(null); setAvailability({}); setLocked(false); setStep(1); };
  ...
  return <Screen scroll avoidKeyboard style={styles.content}>
    ...
    {step === 4 && <><Text style={styles.section}>Tarifs · {selectedCategory?.name}</Text>{(selectedCategory?.rates || []).filter((rate) => rate.active !== false).map((rate) => {
      const rateLabel = rate.name || (rate.rateType === 'public' ? 'Tarif public' : rate.rateType);
      // PHASE-H5 — jamais un libellé fabriqué ("Repas non inclus"/
      // "Conditions communiquées") quand le champ est simplement absent
      // (RatePlan antérieur à H5) : la ligne de conditions est omise plutôt
      // que de prétendre une valeur non prouvée (mission §4/§14).
      const conditionParts = [formatMealPlan(rate.mealPlan), formatCancellationTerms(rate.cancellation)].filter(Boolean);
      return <Pressable accessibilityRole="button" accessibilityLabel={`Choisir le tarif ${rateLabel}, ${formatMoney(rate.amount)} par nuit`} key={rate._id} onPress={() => { setSelectedRate(rate); setStep(5); }} style={styles.card}><Text style={styles.cardTitle}>{rateLabel}</Text><Text style={styles.good}>{formatMoney(rate.amount)} / nuit · {formatMoney(Number(rate.amount) * nights * Number(form.roomsCount))} total</Text>{conditionParts.length > 0 && <Text style={styles.muted}>{conditionParts.join(' · ')}</Text>}</Pressable>; })}<Button label="Changer de catégorie" variant="outline" onPress={() => setStep(3)} /></>}
    {step === 5 && <>{locked && <View style={styles.card} accessibilityLabel="Offre sélectionnée, non modifiable"><Text style={styles.section}>{hotelData?.hotel?.name}</Text><Text style={styles.text}>{selectedCategory?.name} · {selectedRate?.name || (selectedRate?.rateType === 'public' ? 'Tarif public' : selectedRate?.rateType)}</Text><Text style={styles.muted}>{form.checkInDate} → {form.checkOutDate} · {nights} nuit(s) · {form.roomsCount} chambre(s)</Text></View>}<Text style={styles.section}>Informations client</Text><Input label="Prénom" value={form.firstName} onChangeText={set('firstName')} /><Input label="Nom" value={form.lastName} onChangeText={set('lastName')} /><Input label="Email" keyboardType="email-address" autoCapitalize="none" value={form.email} onChangeText={set('email')} /><Input label="Téléphone" keyboardType="phone-pad" value={form.phone} onChangeText={set('phone')} /><Input label="Demandes particulières" value={form.specialRequests} onChangeText={set('specialRequests')} multiline /><Button label="Voir le résumé" disabled={!form.firstName || !form.lastName || !form.email} onPress={() => setStep(6)} />{locked ? <Button label="Retour à la fiche hôtel" variant="outline" onPress={() => navigation.goBack()} /> : <Button label="Retour aux tarifs" variant="outline" onPress={() => setStep(4)} />}</>}
    {step === 6 && <View style={styles.card}><Text style={styles.section}>Résumé</Text><Text style={styles.text}>{hotelData.hotel.name} · {selectedCategory.name}</Text><Text style={styles.text}>{form.checkInDate} → {form.checkOutDate} · {nights} nuit(s)</Text><Text style={styles.text}>{form.roomsCount} chambre(s) · {form.adults} adulte(s) · {form.children} enfant(s)</Text><Text style={styles.text}>{selectedRate.name || selectedRate.rateType}</Text>{formatMealPlan(selectedRate.mealPlan) && <Text style={styles.text}>{formatMealPlan(selectedRate.mealPlan)}</Text>}{formatCancellationTerms(selectedRate.cancellation) && <Text style={styles.text}>{formatCancellationTerms(selectedRate.cancellation)}</Text>}<Text style={styles.total}>{formatMoney(totalPrice)}</Text><Text style={styles.muted}>Aucun paiement n’est prélevé ici. Le règlement s’effectue selon les modalités communiquées par l’hôtel.</Text><Button label="Confirmer la réservation" loading={loading} disabled={loading} onPress={submit} /><Button label="Modifier mes informations" variant="outline" onPress={() => setStep(5)} /><Button label="Abandonner cette demande" variant="outline" onPress={reset} /></View>}
